	...
	var pricingTableGenerator = {
		...
		registerEvents: function() {
			...
			$("iframe[name='preview']").load(function() {
				...
				$(document).on('change keyup', "#Opanda_PricingTablesMetabox input, #Opanda_PricingTablesMetabox select, #Opanda_PricingTablesMetabox textarea", function() {
					self.refreshPreview();
					iframe.__$onp(".content-to-lock").pandalocker('update');
				});
			});
		...
		tableRowConvertToText: function(text) {
			return text.replace(/<div class=\"onp-pl-control-table-row\">([^<]+)<\/div>/gi, '$1\n');
		},
